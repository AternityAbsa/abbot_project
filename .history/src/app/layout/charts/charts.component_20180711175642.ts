...
import { AbbotProcessInterface, CreatedByOrLastModifiedBy, AbbotUserRole} from '../../services/AbbotProcessInterface';
...
export class ChartsComponent implements OnInit {
    ...
    constructor(private processService : ProcessesService, private processModel : ProcessModel, private processInterface: AbbotProcessInterface,
                private createUserModifiedBy: CreatedByOrLastModifiedBy, private userRole: AbbotUserRole, private router : Router, private modalService: NgbModal, private fb: FormBuilder) {
                 this.createForm();
    }
    ...
    this.userRole.id = 1;
    this.createUserModifiedBy.abbotUserRole = this.
    this.processInterface.createdBy = New Creta
    this.processModel.description = "Test5"; 
    this.processModel.lastModifiedBy = [1, ["ROLE_ADMIN"], [1]];
    this.processModel.deleted = false;
    // this.processModel.name = "Muzi";
    // this.processModel.processType = "Test5"; 
    // this.processModel.runmode = "C";
    // this.processModel.sharedObject = 5;
    // this.processModel.version = "A";

    this.processService.createProcess(this.processModel)
    .subscribe(value =>{
      console.log('[POST] created a Process successfully', value);
    }, error => {
      console.log('FAILED to create Process');
    },() =>{
      console.log('POST Create Process - now completed.');
    });
  }
