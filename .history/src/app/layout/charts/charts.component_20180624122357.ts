...
export class ChartsComponent implements OnInit {
    ...
    constructor(private processService : ProcessesService, private processMoed : ProcessModel) {
        }

        loadProcesses(): Observable<any>[]{
            this.processService.getAbBotProcesses().subscribe(
                processes => {
                this.dataModel.UXI = processes['value'].map(blue_applications => blue_applications.UXI);
