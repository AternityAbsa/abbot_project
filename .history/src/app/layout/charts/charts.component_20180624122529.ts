    ...
    constructor(private processService : ProcessesService, private processModel : ProcessModel) {
        ...
        loadProcesses(): Observable<any>[]{
            this.processService.getAbBotProcesses().subscribe(
                ...
                this.processModel.processName = processes['value'].map(processes => processes.proces);
                this.processModel.processStatus = processes['value'].map(processes => processes.processStatusI);
