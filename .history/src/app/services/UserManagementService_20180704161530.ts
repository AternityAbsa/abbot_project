...
import { BASE_PATH, ABBOT_USER_MANAGEMENT_API, ABBOT_USER_MANAGEMENT_API, ABBOT_USER_MANAGEMENT_API, ABBOT_USER_MANAGEMENT_API } from '../services/services';
...
export class UserManagementService {
    ...
    public getAllUsers(){
     ...
     public createUser(): Observable<any[]>{
        return this.http.post(this.user_management_basepath + this.user_management_rest_api,  { headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
     ...
     updateProcess(){
        return this.http.put(this.user_management_basepath + this.user_management_rest_api,  { headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
