...
import { BASE_PATH, ABBOT_AUDIT_CONTROLLER_API } from '../services/services';
...
export class CredentialsManagerService {
    ...
    public getCredentials(){
        return this.http.get(BASE_PATH + ABBOT_AUDIT_CONTROLLER_API, { headers: this.headers})
     ...
     public createUser(credentialsModel: UserManagementModel){
        return this.http.post(BASE_PATH + ABBOT_AUDIT_CONTROLLER_API, JSON.stringify(userModel),  { headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
