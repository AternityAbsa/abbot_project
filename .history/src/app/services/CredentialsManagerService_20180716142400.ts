...
export class CredentialsManagerService {
    ...
    public getCredentials(){
        return this.http.get(BASE_PATH + ABBOT_CREDENTIALS_MANAGEMENT_API, { headers: this.headers})
     ...
     public createUser(credentialsModel: CredentialsManagerModel){
        return this.http.post(BASE_PATH + ABBOT_CREDENTIALS_MANAGEMENT_API, JSON.stringify(userModel),  { headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
     ...
     updateUser(userModel: UserManagementModel){
        return this.http.put(BASE_PATH + ABBOT_CREDENTIALS_MANAGEMENT_API, JSON.stringify(userModel), { headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
     ...
     deleteUser(userId: number){
         console.log(userId);
        return this.http.delete(BASE_PATH + ABBOT_CREDENTIALS_MANAGEMENT_API + userId, { headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
