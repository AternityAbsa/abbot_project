...
export class ProcessStepControllerService {
    ...
    public getAllProcessSteps(){
     ...
     public createProcessStep(): Observable<any[]>{
        return this.http.post(BASE_PATH + ABBOT_PROCESS_MANAGEMENT_API,{ headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
     ...
     updateProcess(){
        return this.http.put(BASE_PATH + ABBOT_PROCESS_STEP_MANAGEMENT_API,{ headers: this.headers})
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
