...
export class ProcessStepControllerService {
    ...
    public getAllUsers(){
        return this.http.get(this.BASE_URL + this.ABBOT_ACTION_MANAGEMENT_API, { headers: this.headers})
     ...
     public createUser(user: UserManagementModel): Observable<any[]>{
        return this.http.post(this.BASE_URL + this.ABBOT_ACTION_MANAGEMENT_API, JSON.stringify(user))
        .map((response: Response) => {
            if (response.status === 204) {
                return undefined;
            } else if (response.status === 500) {
